...
def get_structure_data(request):
    ...
    all_menu = menu[settings.ALL_MENU_KEY]                # 所有菜单list
    permission_url = menu[settings.PERMISSION_MENU_KEY]   # 用户权限菜单list

    # all_menu = [
    #     {'id': 1, 'title': '订单管理', 'parent_id': None},
    #     {'id': 2, 'title': '库存管理', 'parent_id': None},
    #     {'id': 3, 'title': '生产管理', 'parent_id': None},
    #     {'id': 4, 'title': '生产调查', 'parent_id': 3}
    # ]
    ...
    #     4: {'id': 4, 'title': '生产调查', 'parent_id': 3, 'status': False, 'open': False, 'children': []}
    # }
    ...
    for url in permission_url:
        # 添加两个状态：显示 和 展开
        url['status'] = True    # 用户权限菜单都显示
        pattern = url['url']    # 获取url与请求url进行正则匹配
        ...
        pid = url['menu_id']
        while pid:
            all_menu_dict[pid]['status'] = True     # url所属菜单显示
            pid = all_menu_dict[pid]['parent_id']   # 获取父级菜单
    ...
    # 整理菜单层级结构：没有parent_id 的为根菜单，并将有parent_id 的菜单项加入其父项的children内
    menu_data = []
    for i in all_menu_dict:
        if all_menu_dict[i]['parent_id']:               # 有父级菜单的情况
            pid = all_menu_dict[i]['parent_id']
            parent_menu = all_menu_dict[pid]            # 获取父级菜单数据
...
def get_menu_html(menu_data):
    ...
    """
     menu_data = [
        ...
        {'id': 2, 'title': '库存管理', 'parent_id': None, 'status': True, 'open': False, 
        'children': [{'title': '查看库存清单', 'url': '/stock/detail/', 'menu_id': 2, 'status': True, 'open': False}, 
                    {'title': '入库', 'url': '/stock/in/', 'menu_id': 2, 'status': True, 'open': False}]}, 
        ...
        'children': [{'title': '查看生产订单', 'url': '/produce/detail/', 'menu_id': 3, 'status': True, 'open': False}, 
                    ...
                    {'id': 4, 'title': '生产调查', 'parent_id': 3, 'status': True, 'open': False, 
