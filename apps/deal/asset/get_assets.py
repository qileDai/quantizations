...
class GetAssets(object):
    ...
    def showassets(self):
        ...
        for lastday_asset in lastday_obj:
            ...
            lastday_assets += round(float(lastday_asset.lastday_assets), 2)*float(lastday_asset.last)
        ...
        for queryset in show_currency:
            ...
            if last:
                ...
                assets_dict[currency]['last'] = 1
                profit_loss_dict[currency]['last'] = 1
            else:
            ...
            # 当前总资产
            current_total += round(float(balance_info[currency]['total']), 2) * float(assets_dict[currency]['last'])
            # 账户初始总资产
            original_total += round(float(queryset.original_assets), 2) * float(queryset.last)
            # 损益表-当前总资产
            ...
            print(balance_info[currency]['total'], queryset.original_assets, assets_dict[currency]['last'])
